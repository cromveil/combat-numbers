...
public record Rule(WhenCondition when, Style then) {
	...
	static final class StyleCodec {
		static final Codec<Style> CODEC = RecordCodecBuilder.<Style>create(instance ->
			instance.group(
				Identifier.CODEC.optionalFieldOf("skin").forGetter(s -> Optional.ofNullable(s.skinId() != null ? ResourceIds.to(s.skinId()) : null)),
				Identifier.CODEC.optionalFieldOf("animation").forGetter(s -> Optional.ofNullable(s.animationId() != null ? ResourceIds.to(s.animationId()) : null))
			).apply(instance, (skin, animation) ->
				new Style(
					skin.map(ResourceIds::from).orElse(null),
					animation.map(ResourceIds::from).orElse(null)
				)
			)
		);
	}
